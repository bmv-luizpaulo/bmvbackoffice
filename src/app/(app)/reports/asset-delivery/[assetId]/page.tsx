...
function AssetDeliveryReportContent() {
  ...
  const handleGeneratePdf = async () => {
    ...
    try {
      ...
      pdf.save(`Termo_Entrega_${asset?.name?.replace(/\s/g, '_') || assetId}.pdf`);
    } finally {
  ...
  return (
    <div className="min-h-screen bg-gray-100 print:bg-white font-sans">
      <header className="bg-gray-100 p-4 print:hidden flex items-center justify-between sticky top-0 z-20">
        <h2 className='font-headline text-lg'>Preview: Termo de Entrega</h2>
        ...
        <div ref={reportRef} className="mx-auto max-w-4xl bg-white p-12 shadow-lg print:shadow-none A4-container">
          ...
          <section className="mt-8 space-y-6 text-gray-800">
            ...
            <div className="space-y-4 rounded-lg border bg-gray-50 p-6">
              <div>
                ...
                <div className="mt-2 grid grid-cols-2 gap-x-6 gap-y-2 text-sm">
                  ...
                  <p><strong>Status na Entrega:</strong> {asset.status}</p>
                </div>
              </div>

              <div>
                <h2 className="font-headline font-semibold text-lg text-gray-700">Dados do Responsável</h2>
                <div className="mt-2 grid grid-cols-2 gap-x-6 gap-y-2 text-sm">
                  <p><strong>Nome:</strong> {assignee.name}</p>
            ...
            <div className="space-y-2">
              <h3 className="font-headline font-semibold text-lg text-gray-700">Declaração de Responsabilidade</h3>
              <p className="text-sm leading-relaxed">
                O(A) RESPONSÁVEL declara ter recebido o ativo em perfeitas condições de uso e funcionamento, e compromete-se a zelar por sua guarda, conservação e integridade. O ativo destina-se exclusivamente para atividades profissionais, sendo vedado o seu uso para fins pessoais ou a cessão a terceiros. Em caso de dano, perda, roubo ou furto, o(a) RESPONSÁVEL deverá comunicar o fato imediatamente à CONCEDENTE.
              </p>
            </div>
