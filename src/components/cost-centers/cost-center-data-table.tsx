...
export function CostCenterDataTable() {
  ...
  const { data: costCentersData, isLoading } = useCollection<CostCenter>(costCentersQuery);
  ...
  const handleSaveCostCenter = React.useCallback(async (costCenterData: Omit<CostCenter, 'id'>, costCenterId?: string) => {
    ...
    if (costCenterId) {
        ...
        await updateDocumentNonBlocking(costCenterRef, costCenterData);
        toast({ title: "Centro de Custo Atualizado", description: `O centro de custo "${costCenterData.name}" foi atualizado.` });
    } else {
        await addDocumentNonBlocking(collection(firestore, 'costCenters'), costCenterData);
        toast({ title: "Centro de Custo Criado", description: `O centro de custo "${costCenterData.name}" foi criado com sucesso.` });
    }
    setIsFormOpen(false);
  }, [firestore, toast]);
